...
tmp_data <- read.csv("Material 02 - 7 – C - IR - Dados.csv")
...
set.seed(4)
indices <- createDataPartition(data$sonegador, p=0.80,list=FALSE)
treino <- data[indices,] 
teste <- data[-indices,]
...
set.seed(4)
best_model_rf <- train(sonegador~., data=data, method="rf", trControl=ctrl, tuneGrid=tuneGrid)
best_model_rf
