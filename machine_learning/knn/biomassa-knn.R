...
set.seed(4)
ind <- createDataPartition(data$biomassa, p=0.80, list = FALSE)
treino <- data[ind,]
teste <- data[-ind,]
...
Syx(teste$biomassa, predict.knn, length(predict.knn))
mae(teste$biomassa, predict.knn)
cor(teste$biomassa, predict.knn, method="pearson")


###SALVAR O MODELO
saveRDS(knn,"~/Documents/github/IAUFPR/machine_learning/knn/biomassa-knn.rds")
